 ...
 *
 * Usage with Etherscan API to fetch verified ABI:
 *   const ContractABIFetcher = require('./contract-abi');
 *   const fetcher = new ContractABIFetcher(process.env.ETHERSCAN_API_KEY);
 *   const result = await fetcher.getContractABI(deployedAddress);
 */
...
// To regenerate: solc --optimize --optimize-runs 999999 --evm-version paris --bin contracts/MultiSignatureWallet.sol
...
class MultiSigWallet {
   ...
   *
   * Only HTTPS RPC endpoints are supported to ensure secure communication.
   *
   * @param {string} contractAddress  - Deployed contract address (0x-prefixed)
   * @param {string} [rpcUrl]         - HTTPS JSON-RPC endpoint (defaults to Ethereum mainnet)
  ...
  constructor(contractAddress, rpcUrl = 'https://ethereum.publicnode.com', requestTimeout = 30000) {
    ...
    if (!rpcUrl.startsWith('https://')) {
      throw new Error('rpcUrl must use HTTPS for secure communication');
    }
  ...
  /**
   * Makes a JSON-RPC request over HTTPS.
   * @param {string} method
   * @param {Array}  params
   * @returns {Promise<*>}
   * @private
